...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const card of feed.cards) {
    ...
    seen.add(url);
    catalogRoutes.push(entry(`/${card.sellSlug}`, new Date(card.updatedAt), "daily", 0.8));
  }
  ...
  for (const page of feed.landingPages) {
    ...
    if (seen.has(url)) continue;
    seen.add(url);
    catalogRoutes.push(entry(`/${page.slug}`, new Date(page.updatedAt), "weekly", 0.75));
  }

  const insightRoutes: MetadataRoute.Sitemap = feed.insightPosts.map((post) =>
    entry(`/insights/${post.slug}`, new Date(post.lastModified), "weekly", 0.7)
  );

  return [...staticRoutes, ...catalogRoutes, ...insightRoutes];
}
